...
WIFI_StatusTypeDef WIFI_SPI_Transmit(WIFI_HandleTypeDef* hwifi, char* buffer, uint16_t size){
  ...
  while(1);
	...
	if ( !(size % 2) ) strcat(bTx, (const char*) WIFI_TX_PADDING); // If buffer had an odd amount of bytes, append a filler char to bTx
...
WIFI_StatusTypeDef WIFI_Init(WIFI_HandleTypeDef* hwifi)
{
  ...
  WIFI_ENABLE_NSS();
    ...
    temp = (uint16_t)SPI3->DR;
    terminal("temp: %X\n", temp);
//    Status = HAL_SPI_Receive((hwifi->handle) , &Prompt[count], 1, 100);  
//    count += 2;
...
WIFI_StatusTypeDef WIFI_SendATCommand(WIFI_HandleTypeDef* hwifi, char* bCmd, uint16_t sizeCmd, char* bRx, uint16_t sizeRx){
	...
	if(WIFI_IS_READY()) Error_Handler(); // If CMDDATA_READY is still high, then the buffer is too small for the data
